...
internal static class GHashArm
{
	...
	internal static void AppendPaddedSegments(ref VectorBuffer16 accumulator, in VectorBuffer16 key, scoped ReadOnlySpan<byte> first, scoped ReadOnlySpan<byte> second, scoped ReadOnlySpan<byte> third)
	{
		...
		Vector128<byte> internalKey = AdvSimd.Arm64.ReverseElementBits(key.V128);
		Vector128<byte> internalAccumulator = AdvSimd.Arm64.ReverseElementBits(accumulator.V128);
		...
		try
		{
			...
			accumulator.V128 = AdvSimd.Arm64.ReverseElementBits(internalAccumulator);
		}
		finally
		{
	...
	private static void AppendSequential(ref Vector128<byte> accumulator, in Vector128<byte> key, scoped ReadOnlySpan<byte> source)
	{
		...
		{
			Vector128<byte> block = AdvSimd.Arm64.ReverseElementBits(source.AsVectorBuffer16().V128);
			accumulator = GFMultiply(block ^ accumulator, key);
			source = source.Slice(BlockSize);
		}
	}

	private static void AppendPaddedSegment(ref Vector128<byte> accumulator, in Vector128<byte> key, scoped ReadOnlySpan<byte> source, ref VectorBuffer16 finalBlock)
	{
	...
	[SkipLocalsInit]
	private static void AppendPaddedSegmentsFolded(ref VectorBuffer16 accumulator, in VectorBuffer16 key, scoped ReadOnlySpan<byte> first, scoped ReadOnlySpan<byte> second, scoped ReadOnlySpan<byte> third)
	{
		Vector128<byte> internalKey = AdvSimd.Arm64.ReverseElementBits(key.V128);
		GHashArmFoldedState state = new(internalKey, AdvSimd.Arm64.ReverseElementBits(accumulator.V128));
		Unsafe.SkipInit(out VectorBuffer16 finalBlock);
		...
		try
		{
			...
			accumulator.V128 = AdvSimd.Arm64.ReverseElementBits(state.GetAccumulator());
		}
		finally
		{
			state.ZeroMemory();
