...
internal struct GHashVector512State
{
	...
	private void AppendFoldedTail(scoped ReadOnlySpan<byte> source, ref VectorBuffer16 finalBlock, int blockCount)
	{
		ref byte ptr = ref source.GetReference();
		Vector512<byte> blocks = Vector512.LoadUnsafe(ref ptr).ReverseEndianness128();
		blocks = Avx512F.InsertVector128(blocks, blocks.GetLower().GetLower() ^ _accumulator, 0);
		...
		Vector512<byte> lastBlocks = Vector512.Create(lastTwo, Vector256.Create(lastSource, finalBlock.V128)).ReverseEndianness128();
