...
internal struct GHashVector128State
{
	...
	private void AppendFoldedRemainder(scoped ReadOnlySpan<byte> source, scoped ReadOnlySpan<byte> suffix)
	{
		...
		Vector128<byte> block = sourceBlocks is 0 ? suffix.AsVectorBuffer16().V128.ReverseEndianness128() : source.AsVectorBuffer16().V128.ReverseEndianness128();
		...
		{
			block = suffix.AsVectorBuffer16().V128.ReverseEndianness128();
	...
	internal void AppendPaddedSegment(scoped ReadOnlySpan<byte> source, ref VectorBuffer16 finalBlock)
	{
		...
		AppendFoldedRemainder(source.Slice(bulkLength, completeLength - bulkLength), finalBlock);
	}
