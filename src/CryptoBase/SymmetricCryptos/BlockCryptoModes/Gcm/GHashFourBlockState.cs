using static CryptoBase.SymmetricCryptos.BlockCryptoModes.Gcm.GHashX86;

namespace CryptoBase.SymmetricCryptos.BlockCryptoModes.Gcm;

internal struct GHashFourBlockState
{
	...
	internal GHashFourBlockState(Vector128<byte> key, Vector128<byte> accumulator)
	{
		_key1 = key;
	...
	private void AppendBlocks(scoped ReadOnlySpan<byte> source)
	{
		if (IsSupported256)
		{
			AppendBlocks256(source);
		}
		else
		{
			AppendBlocks128(source);
		}
	}

	private void AppendBlocks256(scoped ReadOnlySpan<byte> source)
	{
		int offset = 0;
		...
		if (length is BlockSize)
		{
			...
			_accumulator = GFMultiply(_key1, block ^ _accumulator);
		}
	}

	private void AppendBlocks128(scoped ReadOnlySpan<byte> source)
	{
		int offset = 0;
		int length = source.Length;
		ref byte input = ref source.GetReference();
		...
		while (length >= 4 * BlockSize)
		{
			Vector128<byte> x0 = Vector128.LoadUnsafe(ref input, (nuint)offset).ReverseEndianness128() ^ _accumulator;
	...
	private void AppendFoldedPaddedRemainder(scoped ReadOnlySpan<byte> source, in VectorBuffer16 finalBlock)
	{
		int sourceBlocks = source.Length / BlockSize;
		Debug.Assert(source.Length % BlockSize is 0);
		Debug.Assert(sourceBlocks is >= 0 and < 4);

		Vector128<byte> block = sourceBlocks is 0 ? finalBlock.V128.ReverseEndianness128() : source.AsVectorBuffer16().V128.ReverseEndianness128();
		...
		{
			block = finalBlock.V128.ReverseEndianness128();
	...
	internal void AppendPaddedSegment(scoped ReadOnlySpan<byte> source, ref VectorBuffer16 finalBlock)
	{
		...
		AppendFoldedPaddedRemainder(source.Slice(bulkLength, completeLength - bulkLength), in finalBlock);
	}
