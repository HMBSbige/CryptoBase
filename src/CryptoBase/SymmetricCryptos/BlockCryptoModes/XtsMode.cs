	...
	where TBlockCipher : IBlock16Cipher<TBlockCipher>
{
	...
	public void Encrypt(in ReadOnlySpan<byte> iv, in ReadOnlySpan<byte> source, in Span<byte> destination)
	{
		...
		if (left is not 0)
		{
			EncryptCiphertextStealing(tweak, source.Slice(size, left), destination.Slice(size - BlockBytesSize, BlockBytesSize + left));
		}
	}
	...
	public void Decrypt(in ReadOnlySpan<byte> iv, in ReadOnlySpan<byte> source, in Span<byte> destination)
	{
		...
		if (left is not 0)
		{
			DecryptCiphertextStealing(tweak, source.Slice(size, BlockBytesSize + left), destination.Slice(size, BlockBytesSize + left));
		}
	}

	[SkipLocalsInit]
	[MethodImpl(MethodImplOptions.NoInlining)]
	private void EncryptCiphertextStealing(VectorBuffer16 tweak, ReadOnlySpan<byte> sourceTail, Span<byte> destination)
	{
		Span<byte> snapshot = stackalloc byte[2 * BlockBytesSize];
		Span<byte> lastDst = destination.Slice(0, BlockBytesSize);

		lastDst.CopyTo(snapshot);
		sourceTail.CopyTo(snapshot.Slice(BlockBytesSize));

		int left = sourceTail.Length;
		snapshot.Slice(0, left).CopyTo(destination.Slice(BlockBytesSize));
		snapshot.Slice(BlockBytesSize, left).CopyTo(lastDst);

		VectorBuffer16 tmp = lastDst.AsVectorBuffer16() ^ tweak;
		tmp = dataCipher.Encrypt(tmp);
	...
	[SkipLocalsInit]
	[MethodImpl(MethodImplOptions.NoInlining)]
	private void DecryptCiphertextStealing(VectorBuffer16 tweak, ReadOnlySpan<byte> source, Span<byte> destination)
	{
		Span<byte> snapshot = stackalloc byte[2 * BlockBytesSize];
		...
		VectorBuffer16 tmp = snapshot.AsVectorBuffer16() ^ finalTweak;
		tmp = dataCipher.Decrypt(tmp);
		destination.AsVectorBuffer16() = tmp ^ finalTweak;

		int left = source.Length - BlockBytesSize;
		destination.Slice(0, left).CopyTo(destination.Slice(BlockBytesSize));
		snapshot.Slice(BlockBytesSize, left).CopyTo(destination);

		tmp = destination.AsVectorBuffer16() ^ tweak;
		tmp = dataCipher.Decrypt(tmp);
		destination.AsVectorBuffer16() = tmp ^ tweak;
	}
	...
	private static Vector128<byte> GF128MulSse2(Vector128<byte> tweak, [ConstantExpected(Min = 1, Max = 64)] int x)
	{
		if (x is 1)
		{
			Vector128<int> carry = Vector128.Shuffle(tweak.AsInt32(), Vector128.Create(3, 0, 1, 0)) >> 31;
		...
		tmp1 = Sse2.ShiftLeftLogical128BitLane(tmp1, 8);
