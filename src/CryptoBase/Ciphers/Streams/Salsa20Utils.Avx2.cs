...
internal static partial class Salsa20Utils
{
	...
	public static int SalsaCore512(byte rounds, Span<uint> state, ReadOnlySpan<byte> source, Span<byte> destination)
	{
		...
		Vector256<byte> o12 = Vector256.Create(Unsafe.Add(ref stateRef, 12)).AsByte();
		...
		while (length >= 512)
		{
			...
			ChaCha20Utils.AddAndTranspose(counter, out Vector256<byte> o8, out Vector256<byte> o9);
