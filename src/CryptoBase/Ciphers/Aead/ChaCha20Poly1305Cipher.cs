...
/// </summary>
public sealed class ChaCha20Poly1305Cipher : IAeadCipher<ChaCha20Poly1305Cipher>
{
	/// <summary>The required key size, in bytes.</summary>
	public const int KeySize = ChaCha20Cipher.KeySize;

	/// <inheritdoc />
	public static ChaCha20Poly1305Cipher Create(scoped ReadOnlySpan<byte> key)
	{
		return new ChaCha20Poly1305Cipher(key);
	}

	/// <inheritdoc />
	public static int NonceSize => ChaCha20Cipher.IVSize;

	/// <inheritdoc />
	public static int TagSize => 16;

	private readonly ChaCha20Cipher _chacha20;
	...
	public ChaCha20Poly1305Cipher(ReadOnlySpan<byte> key)
	{
		...
		_chacha20 = new ChaCha20Cipher(key, EmptyIV12);
	}
