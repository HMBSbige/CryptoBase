...
public class Poly1305X86 : IMac
{
	...
	public Poly1305X86(ReadOnlySpan<byte> key)
	{
		...
		uint r1 = BinaryPrimitives.ReadUInt32LittleEndian(key[3..]) >> 2 & 0x3FFFF03;
		uint r2 = BinaryPrimitives.ReadUInt32LittleEndian(key[6..]) >> 4 & 0x3FFC0FF;
		uint r3 = BinaryPrimitives.ReadUInt32LittleEndian(key[9..]) >> 6 & 0x3F03FFF;
		uint r4 = BinaryPrimitives.ReadUInt32LittleEndian(key[12..]) >> 8 & 0x00FFFFF;
	...
	{
		Vector128<uint> h01 = IntrinsicsUtils.CreateTwoUInt(a0, a1);
		Vector128<uint> h23 = IntrinsicsUtils.CreateTwoUInt(a2, a3);
		Vector128<uint> h44 = IntrinsicsUtils.CreateTwoUInt(a4, a4);
		...
		out ulong d0, out ulong d1, out ulong d2, out ulong d3, out ulong d4)
	{
		// h0 * r0 + h2 * s3
		// h1 * s4 + h3 * s2
		Vector128<ulong> t00 = Sse2.Multiply(h01, _r0s4);
		Vector128<ulong> t01 = Sse2.Multiply(h23, _s3s2);
		Vector128<ulong> t1 = Sse2.Add(t01, t00);
		// h0 * r1 + h2 * s4
		// h1 * r0 + h3 * s3
		t00 = Sse2.Multiply(h01, _r1r0);
		t01 = Sse2.Multiply(h23, _s4s3);
		Vector128<ulong> t2 = Sse2.Add(t01, t00);
		// h4 * s1
		// h4 * s2
		Vector128<ulong> t3 = Sse2.Multiply(h44, _s1s2);
		// d0 = t1[0] + t1[1] + t3[0]
		// d1 = t2[0] + t2[1] + t3[1]
		Vector128<ulong> t = Sse2.UnpackLow(t1, t2) + Sse2.UnpackHigh(t1, t2) + t3;
		d0 = t.ToScalar();
		...
		// d2 = t1[0] + t1[1] + t3[0]
		// d3 = t2[0] + t2[1] + t3[1]
		t = Sse2.UnpackLow(t1, t2) + Sse2.UnpackHigh(t1, t2) + t3;
		d2 = t.ToScalar();
		...
		// d4 = t1[0] + t1[1] + t3[0]
		d4 = (t1 + Sse2.ShiftRightLogical128BitLane(t1, 8) + t3).ToScalar();
	}
	...
	private void Block(ReadOnlySpan<byte> m)
	{
		...
		Vector128<uint> h44 = IntrinsicsUtils.CreateTwoUInt(_h4, _h4);

		Vector128<uint> m06 = IntrinsicsUtils.CreateTwoUInt(BinaryPrimitives.ReadUInt32LittleEndian(m) & 0x3ffffff, BinaryPrimitives.ReadUInt32LittleEndian(m[3..]) >> 2 & 0x3ffffff);
		h01 = Sse2.Add(h01, m06);
		Vector128<uint> m612 = IntrinsicsUtils.CreateTwoUInt(BinaryPrimitives.ReadUInt32LittleEndian(m[6..]) >> 4 & 0x3ffffff, BinaryPrimitives.ReadUInt32LittleEndian(m[9..]) >> 6 & 0x3ffffff);
		h23 = Sse2.Add(h23, m612);
		uint m4 = BinaryPrimitives.ReadUInt32LittleEndian(m[12..]) >> 8 | 1u << 24;
	...
	private void Block2(ReadOnlySpan<byte> m)
	{
		...
		ReadOnlySpan<uint> n1 = MemoryMarshal.Cast<byte, uint>(m[3..]);
		...
		ReadOnlySpan<uint> n2 = MemoryMarshal.Cast<byte, uint>(m[6..]);
		...
		ReadOnlySpan<uint> n3 = MemoryMarshal.Cast<byte, uint>(m[9..]);
		...
		ReadOnlySpan<uint> n4 = MemoryMarshal.Cast<byte, uint>(m[12..]);
