using CryptoBase.Abstractions.SymmetricCryptos;
using CryptoBase.DataFormatExtensions;
using CryptoBase.SymmetricCryptos.BlockCryptoModes;
using CryptoBase.SymmetricCryptos.BlockCryptos.AES;
using CryptoBase.SymmetricCryptos.StreamCryptos;
using System.Security.Cryptography;

namespace CryptoBase.Tests;

public class AESCTRTest
{
	/// <summary>
	/// https://gchq.github.io/CyberChef/#recipe=AES_Encrypt(%7B'option':'Hex','string':'00000000000000000000000000000000'%7D,%7B'option':'Hex','string':'00000000000000000000000000000000'%7D,'CTR','Hex','Hex','')
	/// </summary>
	public static readonly TheoryData<string, string, string, string> Data = new()
	{
		{ @"A0B0C0D0E0F1011121314151617181AA", @"fe3577795961e1fbbbb82528c74d2e99", @"", @"" },
		{ @"000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F", @"00112233445566778899aabbccddeeff", @"", @"" },
		{ @"08090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F", @"00112233445566778899aabbccddeeff", @"", @"" }
	};

	private static void Test_Internal(IStreamCrypto crypto, string hex, string hex2)
	{
		Assert.Equal(@"AES-CTR", crypto.Name);

		Span<byte> h1 = hex.FromHex();
		Span<byte> h2 = hex2.FromHex();

		Span<byte> o = stackalloc byte[h1.Length];

		crypto.Update(h1, o);
		Assert.Equal(h2, o);

		crypto.Reset();

		crypto.Update(h2[..73], o);
		crypto.Update(h2[73..], o[73..]);
		Assert.Equal(h1, o);

		crypto.Reset();

		for (int i = 0; i < h1.Length; ++i)
		{
			crypto.Update(h1.Slice(i, 1), o.Slice(i, 1));
		}

		Assert.Equal(h2, o);

		crypto.Dispose();
	}

	[Theory]
	[MemberData(nameof(Data))]
	public void Test(string keyHex, string ivHex, string hex, string hex2)
	{
		byte[] key = keyHex.FromHex();
		byte[] iv = ivHex.FromHex();
		Test_Internal(StreamCryptoCreate.AesCtr(key, iv), hex, hex2);
		Test_Internal(new CtrMode128Ctr32<AesCipher>(AesCipher.Create(key), iv), hex, hex2);
	}

	[Theory]
	[InlineData(2 * 16 - 1)]
	[InlineData(4 * 16 - 1)]
	[InlineData(8 * 16 - 1)]
	[InlineData(16 * 16 - 1)]
	[InlineData(32 * 16 - 1)]
	[InlineData(64 * 16 - 1)]
	[InlineData(128 * 16 - 1)]
	[InlineData(256 * 16 - 1)]
	[InlineData(512 * 16 - 1)]
	public void TestBlocks(int length)
	{
		using IStreamCrypto aesctr = new CtrMode128<AesCipher>(AesCipher.Create(RandomNumberGenerator.GetBytes(16)), RandomNumberGenerator.GetBytes(16));
		TestUtils.TestBlocks(aesctr, length);

		using IStreamCrypto aesctr32 = new CtrMode128Ctr32<AesCipher>(AesCipher.Create(RandomNumberGenerator.GetBytes(16)), RandomNumberGenerator.GetBytes(16));
		TestUtils.TestBlocks(aesctr32, length);
	}
}
