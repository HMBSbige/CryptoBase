...
public class ChaCha20Poly1305Test
{
	...
	public async Task Test(string keyHex, string nonceHex, string associatedDataHex, string tagHex, string plainHex, string cipherHex)
	{
		...
		await ChaCha20Poly1305Cipher.Create(key).AeadTest(nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
	}

	[Test]
	public async Task SupportsUnalignedSegments()
	{
		byte[] key = RandomNumberGenerator.GetBytes(32);
		byte[] nonce = RandomNumberGenerator.GetBytes(ChaCha20Poly1305Cipher.NonceSize);
