...
public class AesGcmTest
{
	...
	public async Task Test(string keyHex, string nonceHex, string associatedDataHex, string tagHex, string plainHex, string cipherHex)
	{
		...
		await GcmMode128<AesCipher>.Create(key).AeadTest(nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
	}
	...
	public async Task TagLengthMustBeExactAndRejectedCallsDoNotChangeStateOrOutput(int tagLength)
	{
		...
		expectedTag.AsSpan().Slice(0, Math.Min(expectedTag.Length, invalidDecryptionTag.Length)).CopyTo(invalidDecryptionTag);
		byte[] invalidDecryptionTagCopy = invalidDecryptionTag.ToArray();
		PrepareDestination(invalidDecryptionDestination);

		await Assert.That(() => gcm.TryDecrypt(nonce, expectedCiphertext, invalidDecryptionTag, invalidDecryptionDestination, associatedData)).ThrowsExactly<ArgumentOutOfRangeException>();
		...
		await Assert.That(gcm.TryDecrypt(nonce, expectedCiphertext, expectedTag, actualPlaintext, associatedData)).IsTrue();
