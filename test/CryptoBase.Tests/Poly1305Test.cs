using CryptoBase.Abstractions;
using CryptoBase.DataFormatExtensions;
using CryptoBase.Macs.Poly1305;

namespace CryptoBase.Tests;

public class Poly1305Test
{
	/// <summary>
	/// https://tools.ietf.org/html/rfc8439
	/// </summary>
	public static IEnumerable<(string, string, string)> Data =>
	[
		(@"85D6BE7857556D337F4452FE42D506A80103808AFB0DB2FD4ABFF6AF4149F51B", @"43727970746F6772617068696320466F72756D2052657365617263682047726F7570", @"c88886f51af32a75f0fdf57c4a7defdd"),
		(@"746869732069732033322d62797465206b657920666f7220506f6c7931333035", @"48656c6c6f20776f726c6421", @"cf76f9033b46a114c8ba0577105cc8ed"),
		(@"746869732069732033322d62797465206b657920666f7220506f6c7931333035", @"85D6BE7857556D337F4452FE42D506A80103808AFB0DB2FD4ABFF6AF4149F51B746869732069732033322d62797465206b657920666f7220506f6c793133303543727970746F6772617068696320466F72756D2052657365617263682047726F75708A438BDEE65C422A8366A2F85E5E93972FF925F667EB483EB01B1CD6C9", @"b0c63d86c91980a61c0824fb5cf618cc"),
		(@"746869732069732033322d62797465206b657920666f7220506f6c7931333035", @"ec74691700388dace60b6a392f328c2b971b2f952b2a56a5604ac0b66e94bd4ef8a2c3b884cfa59ca342b2e3da53ec1d3b69b62c9a392687aaf55d95a1df6b0ad2c55bb64fc4802cc3feda602b6656a05b40b6e7ad2c55bb64f62882c85b0685353deb7f38cbb1ad69223dcc3457ae5b6b0dfa6bf4ded81d", @"c783ec8f3716299f4e74a76f4e03296b")
	];

	private static (string Name, int Length, bool First, bool Second, bool Third) Test_Internal<T>(T mac, string plainHex, string cipherHex) where T : IMac, allows ref struct
	{
		byte[] plain = plainHex.FromHex();

		byte[] cipher = cipherHex.FromHex();

		byte[] o = new byte[mac.Length];

		string name = mac.Name;
		int length = mac.Length;

		mac.Update(plain);
		mac.GetMac(o);

		bool first = cipher.SequenceEqual(o);

		mac.Update(plain);
		mac.GetMac(o);

		bool second = cipher.SequenceEqual(o);

		mac.Update(plain);
		mac.Reset();

		mac.Update(plain);
		mac.GetMac(o);

		bool third = cipher.SequenceEqual(o);

		mac.Dispose();

		return (name, length, first, second, third);
	}

	[Test]
	[MethodDataSource(nameof(Data))]
	public async Task Test(string keyHex, string plainHex, string cipherHex)
	{
		byte[] key = keyHex.FromHex();

		foreach ((string Name, int Length, bool First, bool Second, bool Third) result in new[] { Test_Internal(new Poly1305(key), plainHex, cipherHex), Test_Internal(new Poly1305SF(key), plainHex, cipherHex) })
		{
			await Assert.That(result.Name).IsEqualTo(@"Poly1305");
			await Assert.That(result.Length).IsEqualTo(16);
			await Assert.That(result.First).IsTrue();
			await Assert.That(result.Second).IsTrue();
			await Assert.That(result.Third).IsTrue();
		}
	}

	[Test]
	[RequiresX86]
	[MethodDataSource(nameof(Data))]
	public async Task TestX86(string keyHex, string plainHex, string cipherHex)
	{
		byte[] key = keyHex.FromHex();
		(string Name, int Length, bool First, bool Second, bool Third) result = Test_Internal(new Poly1305X86(key), plainHex, cipherHex);

		await Assert.That(result.Name).IsEqualTo(@"Poly1305");
		await Assert.That(result.Length).IsEqualTo(16);
		await Assert.That(result.First).IsTrue();
		await Assert.That(result.Second).IsTrue();
		await Assert.That(result.Third).IsTrue();
	}
}
