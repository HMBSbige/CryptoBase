using CryptoBase.Abstractions.SymmetricCryptos;
using CryptoBase.DataFormatExtensions;
using CryptoBase.SymmetricCryptos.BlockCryptoModes;
using CryptoBase.SymmetricCryptos.BlockCryptos.SM4;
using CryptoBase.SymmetricCryptos.StreamCryptos;
using System.Security.Cryptography;

namespace CryptoBase.Tests;

public class SM4CTRTest
{
	/// <summary>
	/// https://gchq.github.io/CyberChef/#recipe=SM4_Encrypt(%7B'option':'Hex','string':''%7D,%7B'option':'Hex','string':''%7D,'CTR','Hex','Hex')Remove_whitespace(true,true,true,true,true,false)To_Upper_case('All')
	/// </summary>
	public static IEnumerable<(string, string, string, string)> Data =>
	[
		("B7FAD2F367B9999C95E207CA2E16616D", "CBD328D0C128002C6A44384590E6DBC9", "", ""),
		("91602034DB05CDF2F47C023F7EB30DAE", "B934FD1AAE4012AB34031BEEFC06E631", "", "")
	];

	private static async Task T(IStreamCrypto crypto, byte[] source, byte[] expected)
	{
		await Assert.That(crypto.Name).IsEqualTo(@"SM4-CTR");

		byte[] destination = new byte[source.Length + 1];

		crypto.Update(source, destination);

		await Assert.That(expected.SequenceEqual(destination.AsSpan().Slice(0, source.Length))).IsTrue();

		crypto.Reset();

		crypto.Update(source, destination);

		await Assert.That(expected.SequenceEqual(destination.AsSpan().Slice(0, source.Length))).IsTrue();

		crypto.Dispose();
	}

	[Test]
	[MethodDataSource(nameof(Data))]
	public async Task Test(string keyHex, string ivHex, string sourceHex, string expectedHex)
	{
		byte[] key = keyHex.FromHex();
		byte[] iv = ivHex.FromHex();
		byte[] source = sourceHex.FromHex();
		byte[] expected = expectedHex.FromHex();

		await T(StreamCryptoCreate.Sm4Ctr(key, iv), source, expected);
		await T(new CtrMode128Ctr32<Sm4Cipher>(Sm4Cipher.Create(key), iv), source, expected);
	}

	[Test]
	[Arguments(2 * 16 - 1)]
	[Arguments(4 * 16 - 1)]
	[Arguments(8 * 16 - 1)]
	[Arguments(16 * 16 - 1)]
	[Arguments(32 * 16 - 1)]
	[Arguments(64 * 16 - 1)]
	[Arguments(128 * 16 - 1)]
	[Arguments(256 * 16 - 1)]
	[Arguments(512 * 16 - 1)]
	public async Task TestBlocks(int length)
	{
		using IStreamCrypto aesctr = new CtrMode128<Sm4Cipher>(Sm4Cipher.Create(RandomNumberGenerator.GetBytes(16)), RandomNumberGenerator.GetBytes(16));
		await TestUtils.TestBlocks(aesctr, length);

		using IStreamCrypto aesctr32 = new CtrMode128Ctr32<Sm4Cipher>(Sm4Cipher.Create(RandomNumberGenerator.GetBytes(16)), RandomNumberGenerator.GetBytes(16));
		await TestUtils.TestBlocks(aesctr32, length);
	}
}
