...
public class Salsa20Test
{
	...
	private static async Task Test_Internal(SnuffleCryptoBase crypto, string hex0, string hex1, string hex2, string hex3)
	{
		await Assert.That(crypto.Name).IsEqualTo(@"Salsa20");
		await Assert.That(crypto.IvSize).IsEqualTo(8);

		byte[] h0 = hex0.FromHex();
		byte[] h1 = hex1.FromHex();
		byte[] h2 = hex2.FromHex();
		byte[] h3 = hex3.FromHex();

		byte[] i = new byte[512];
		byte[] o = new byte[512];

		crypto.Update(i.AsSpan().Slice(0, 63), o);// 0 - 62
		await Assert.That(h0.AsSpan().Slice(0, 63).SequenceEqual(o.AsSpan().Slice(0, 63))).IsTrue();
		crypto.Update(i.AsSpan().Slice(0, 63), o);// 63 - 125
		await Assert.That(o[0]).IsEqualTo(h0[63]);

		// 126 - 65456
		for (int j = 0; j < 1037; j++)
		{
			crypto.Update(i.AsSpan().Slice(0, 63), o);
		}

		crypto.Update(i.AsSpan().Slice(0, 63), o);// 65457 - 65519
		await Assert.That(h1.AsSpan().Slice(0, 48).SequenceEqual(o.AsSpan().Slice(15, 63 - 15))).IsTrue();
		crypto.Update(i.AsSpan().Slice(0, 63), o);// 65520 - 65582
		await Assert.That(h1.AsSpan().Slice(48).SequenceEqual(o.AsSpan().Slice(0, 16))).IsTrue();
		await Assert.That(h2.AsSpan().Slice(0, 47).SequenceEqual(o.AsSpan().Slice(16, 47))).IsTrue();

		crypto.Update(i.AsSpan().Slice(0, 64), o);// 65583 - 65646
		await Assert.That(h2.AsSpan().Slice(47).SequenceEqual(o.AsSpan().Slice(0, 17))).IsTrue();

		// 65647 - 130990
		for (int j = 0; j < 1021; j++)
		{
			crypto.Update(i.AsSpan().Slice(0, 64), o);
		}

		crypto.Update(i.AsSpan().Slice(0, 64), o);// 130991 - 131054
		await Assert.That(h3.AsSpan().Slice(0, 47).SequenceEqual(o.AsSpan().Slice(17, 64 - 17))).IsTrue();
		crypto.Update(i.AsSpan().Slice(0, 64), o);// 131055 - 131118
		await Assert.That(h3.AsSpan().Slice(47).SequenceEqual(o.AsSpan().Slice(0, 17))).IsTrue();

		crypto.Reset();

		crypto.Update(i.AsSpan().Slice(0, 128), o);// 0 - 127
		await Assert.That(h0.SequenceEqual(o.AsSpan().Slice(0, 64))).IsTrue();
		crypto.Update(i.AsSpan().Slice(0, 64), o);// 128 - 191

		// 192 - 65471
		for (int j = 0; j < 510; j++)
		{
			crypto.Update(i.AsSpan().Slice(0, 128), o);
		}

		crypto.Update(i.AsSpan().Slice(0, 128), o);// 65472 - 65599
		await Assert.That(h1.SequenceEqual(o.AsSpan().Slice(0, 64))).IsTrue();
		await Assert.That(h2.SequenceEqual(o.AsSpan().Slice(64, 64))).IsTrue();

		// 65600 - 130879
		for (int j = 0; j < 255; j++)
		{
			crypto.Update(i.AsSpan().Slice(0, 256), o);
		}

		crypto.Update(i.AsSpan().Slice(0, 256), o);// 130880 - 131135
		await Assert.That(h3.SequenceEqual(o.AsSpan().Slice(128, 64))).IsTrue();

		crypto.Reset();
		crypto.Update(i.AsSpan().Slice(0, 512), o);// 0 - 511
		await Assert.That(h0.SequenceEqual(o.AsSpan().Slice(0, 64))).IsTrue();

		// 512 - 65535
		for (int j = 0; j < 127; j++)
		{
			crypto.Update(i.AsSpan().Slice(0, 512), o);
		}

		await Assert.That(h1.SequenceEqual(o.AsSpan().Slice(448, 64))).IsTrue();
		crypto.Update(i.AsSpan().Slice(0, 512), o);// 65536 - 66047
		await Assert.That(h2.SequenceEqual(o.AsSpan().Slice(0, 64))).IsTrue();
		...
		await Assert.That(h3.SequenceEqual(o.AsSpan().Slice(448, 64))).IsTrue();

		crypto.Reset();

		crypto.Update(i.AsSpan().Slice(0, 64), o);// 0 - 63
		await Assert.That(h0.SequenceEqual(o.AsSpan().Slice(0, 64))).IsTrue();
		...
		await Assert.That(h3.SequenceEqual(o.AsSpan().Slice(0, 64))).IsTrue();
	...
	public async Task Test(string keyHex, string ivHex, string hex0, string hex1, string hex2, string hex3)
	{
		byte[] key = keyHex.FromHex();
		byte[] iv = ivHex.FromHex();
		await Test_Internal(new BcSalsa20Crypto(key, iv), hex0, hex1, hex2, hex3);
		await Test_Internal(new Salsa20Crypto(key, iv), hex0, hex1, hex2, hex3);
	}
}
