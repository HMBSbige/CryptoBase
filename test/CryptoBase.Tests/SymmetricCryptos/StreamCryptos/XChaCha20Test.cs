using CryptoBase.Abstractions.SymmetricCryptos;
using CryptoBase.SymmetricCryptos.StreamCryptos;

namespace CryptoBase.Tests.SymmetricCryptos.StreamCryptos;

public class XChaCha20Test
{
	/// <summary>
	/// https://tools.ietf.org/html/draft-irtf-cfrg-xchacha-03#appendix-A.3.2
	/// </summary>
	public static IEnumerable<(string, string, string, string)> DataCounter1 =>
	[
		(@"808182838485868788898a8b8c8d8e8f909192939495969798999a9b9c9d9e9f", @"404142434445464748494a4b4c4d4e4f5051525354555658", @"", @"")
	];

	public static IEnumerable<(string, string, string, string)> DataCounter0 =>
	[
		(@"808182838485868788898a8b8c8d8e8f909192939495969798999a9b9c9d9e9f", @"404142434445464748494a4b4c4d4e4f5051525354555658", @"", @"")
	];

	private static async Task TestCounter1_Internal(SnuffleCryptoBase crypto, string hex, string hex2)
	{
		await Assert.That(crypto.Name).IsEqualTo(@"XChaCha20");
		await Assert.That(crypto.IVSize).IsEqualTo(24);

		byte[] h1 = Convert.FromHexString(hex);
		byte[] h2 = Convert.FromHexString(hex2);

		byte[] i1 = new byte[368];
		byte[] o1 = new byte[i1.Length];

		h1.CopyTo(i1.AsSpan().Slice(64));

		crypto.Update(i1, o1);
		await Assert.That(o1.AsMemory(64, 304)).IsEquivalentTo(h2, CollectionOrdering.Matching);

		crypto.Reset();

		h1.CopyTo(i1.AsSpan().Slice(64));

		crypto.Update(i1, o1);
		await Assert.That(o1.AsMemory(64, 304)).IsEquivalentTo(h2, CollectionOrdering.Matching);

		crypto.Dispose();
	}

	private static async Task TestCounter0_Internal(SnuffleCryptoBase crypto, string hex, string hex2)
	{
		await Assert.That(crypto.Name).IsEqualTo(@"XChaCha20");
		await Assert.That(crypto.IVSize).IsEqualTo(24);

		byte[] h1 = Convert.FromHexString(hex);

		byte[] h2 = Convert.FromHexString(hex2);

		byte[] o1 = new byte[h1.Length];

		crypto.Update(h1, o1);
		await Assert.That(h2).IsEquivalentTo(o1, CollectionOrdering.Matching);

		crypto.Reset();

		crypto.Update(h1, o1);
		await Assert.That(h2).IsEquivalentTo(o1, CollectionOrdering.Matching);

		crypto.Dispose();
	}

	[Test]
	[MethodDataSource(nameof(DataCounter1))]
	public async Task TestCounter1(string keyHex, string ivHex, string hex, string hex2)
	{
		byte[] key = Convert.FromHexString(keyHex);
		byte[] iv = Convert.FromHexString(ivHex);
		await TestCounter1_Internal(new XChaCha20Crypto(key, iv), hex, hex2);
	}

	[Test]
	[MethodDataSource(nameof(DataCounter0))]
	public async Task TestCounter0(string keyHex, string ivHex, string hex, string hex2)
	{
		byte[] key = Convert.FromHexString(keyHex);
		byte[] iv = Convert.FromHexString(ivHex);
		await TestCounter0_Internal(new XChaCha20Crypto(key, iv), hex, hex2);
	}
}
