...
public class Salsa20Test
{
	...
	public async Task VerifiedFirstBlockForBothKeySizes(string keyHex, string nonceHex, string expectedHex)
	{
		...
		using Salsa20Crypto crypto = new(Convert.FromHexString(keyHex), Convert.FromHexString(nonceHex));

		crypto.Update(source, actual);

		await Assert.That(crypto.Name).IsEqualTo("Salsa20");
		await Assert.That(crypto.IVSize).IsEqualTo(8);
		await Assert.That(Convert.ToHexStringLower(actual)).IsEqualTo(expectedHex).IgnoringCase();
	}
}
