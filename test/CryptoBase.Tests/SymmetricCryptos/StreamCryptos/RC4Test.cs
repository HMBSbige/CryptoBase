...
public class RC4Test
{
	private static async Task Test_Internal(IStreamCrypto crypto, int originSize, string hex, string hex2)
	{
		...
		crypto.Update(i, o);
		await Assert.That(h1).IsEquivalentTo(o, CollectionOrdering.Matching);

		crypto.Update(i, o);
		await Assert.That(h2).IsEquivalentTo(o, CollectionOrdering.Matching);

		crypto.Reset();

		crypto.Update(h1, o);
		await Assert.That(i).IsEquivalentTo(o, CollectionOrdering.Matching);
