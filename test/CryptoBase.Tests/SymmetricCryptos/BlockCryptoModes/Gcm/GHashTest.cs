...
namespace CryptoBase.Tests.SymmetricCryptos.BlockCryptoModes.Gcm;
...
public class GHashTest
{
	...
	public async Task ShortDestinationDoesNotModifyOutputOrState()
	{
		...
		try
		{
			...
			await Assert.That(() => hash.HashPaddedSegmentsAndReset(second, default, default, shortDestination)).Throws<ArgumentOutOfRangeException>();
	...
	public async Task KeyMustBeExactlyOneBlock(int keyLength)
	{
		...
		await Assert.That(() => GHashAlgorithmCore.Create(key)).ThrowsExactly<ArgumentOutOfRangeException>();
	}
