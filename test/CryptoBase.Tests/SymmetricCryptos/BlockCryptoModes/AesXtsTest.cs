...
public class AesXtsTest
{
	...
	public async Task ExactInPlaceMatchesOpenSslAtCiphertextStealingBoundaries(string plaintextHex, string ciphertextHex)
	{
		...
		using XtsMode<AesCipher> crypto = Create(Convert.FromHexString("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"));
	...
	public async Task InvalidInputsAreRejectedBeforeWriting(bool decrypt)
	{
		...
		using XtsMode<AesCipher> crypto = Create(CreateDeterministicSource(32));

		destination.AsSpan().Fill(DestinationSentinel);
		await Assert.That(() => Transform(crypto, decrypt, iv.AsSpan(0, 15), source, destination))
			.ThrowsExactly<ArgumentOutOfRangeException>();
		await Assert.That(destination).All(static value => value is DestinationSentinel);

		await Assert.That(() => Transform(crypto, decrypt, iv, source.AsSpan(0, 15), destination))
			.ThrowsExactly<ArgumentOutOfRangeException>();
		await Assert.That(destination).All(static value => value is DestinationSentinel);

		await Assert.That(() => Transform(crypto, decrypt, iv, source, destination.AsSpan(0, 15)))
			.ThrowsExactly<ArgumentOutOfRangeException>();
		await Assert.That(destination).All(static value => value is DestinationSentinel);
	}
	...
	private static async Task VerifyVector(string keyHex, byte[] iv, string plaintextHex, string ciphertextHex)
	{
		...
		destination.AsSpan().Fill(DestinationSentinel);
		using XtsMode<AesCipher> crypto = Create(key);

		crypto.Encrypt(iv, plaintext, destination);

		await Assert.That(crypto.Name).IsEqualTo("AES-XTS");
		await Assert.That(crypto.BlockSize).IsEqualTo(16);
		await Assert.That(crypto.GetMaxByteCount(plaintext.Length)).IsEqualTo(plaintext.Length);
		await AssertOutput(destination, ciphertext, ciphertext.Length);

		destination.AsSpan().Fill(DestinationSentinel);
		crypto.Decrypt(iv, ciphertext, destination);
		await AssertOutput(destination, plaintext, plaintext.Length);
	}

	private static XtsMode<AesCipher> Create(byte[] key)
	{
		int halfLength = key.Length / 2;
		return new XtsMode<AesCipher>(AesCipher.Create(key.AsSpan(0, halfLength)), AesCipher.Create(key.AsSpan(halfLength)));
	}

	private static void Transform(IBlockModeOneShot crypto, bool decrypt, ReadOnlySpan<byte> iv, ReadOnlySpan<byte> source, Span<byte> destination)
	{
		if (decrypt)
		{
