...
public class AesCtrTest
{
	...
	[Arguments(32)]
	[Arguments(128)]
	public async Task StandardVectorMatchesCounterWidth(int counterSize)
	{
		...
		await VerifyStreamVector(counterSize is 32 ? new CtrMode128Ctr32<AesCipher>(AesCipher.Create(key), counter) : StreamCryptoCreate.AesCtr(key, counter), "AES-CTR", plaintext, ciphertext);
	}
}
