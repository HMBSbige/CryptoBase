using CryptoBase.Abstractions.SymmetricCryptos;
using CryptoBase.SymmetricCryptos.AeadCryptos;
using System.Diagnostics.CodeAnalysis;
using System.Security.Cryptography;

namespace CryptoBase.Tests.SymmetricCryptos.AeadCryptos;

public class ChaCha20Poly1305Test
{
	private static async Task Test_Internal(IAeadCrypto crypto, string nonceHex, string associatedDataHex, string tagHex, string plainHex, string cipherHex)
	{
		await crypto.AeadTest(@"ChaCha20-Poly1305", nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
	}

	/// <summary>
	/// https://tools.ietf.org/html/rfc8439
	/// </summary>
	[Test]
	[Arguments
	(
		@"808182838485868788898A8B8C8D8E8F909192939495969798999A9B9C9D9E9F",
		@"070000004041424344454647",
		@"50515253C0C1C2C3C4C5C6C7",
		@"1AE10B594F09E26A7E902ECBD0600691",
		@"4C616469657320616E642047656E746C656D656E206F662074686520636C617373206F66202739393A204966204920636F756C64206F6666657220796F75206F6E6C79206F6E652074697020666F7220746865206675747572652C2073756E73637265656E20776F756C642062652069742E",
		@"D31A8D34648E60DB7B86AFBC53EF7EC2A4ADED51296E08FEA9E2B5A736EE62D63DBEA45E8CA9671282FAFB69DA92728B1A71DE0A9E060B2905D6A5B67ECD3B3692DDBD7F2D778B8C9803AEE328091B58FAB324E4FAD675945585808B4831D7BC3FF4DEF08E4B7A9DE576D26586CEC64B6116"
	)]
	[Arguments
	(
		@"1C9240A5EB55D38AF333888604F6B5F0473917C1402B80099DCA5CBC207075C0",
		@"000000000102030405060708",
		@"F33388860000000000004E91",
		@"EEAD9D67890CBB22392336FEA1851F38",
		@"",
		@""
	)]
	public async Task Test(string keyHex, string nonceHex, string associatedDataHex, string tagHex, string plainHex, string cipherHex)
	{
		byte[] key = Convert.FromHexString(keyHex);
		await Test_Internal(new DefaultChaCha20Poly1305Crypto(key), nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
		await Test_Internal(new ChaCha20Poly1305Crypto(key), nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
		await Test_Internal(AeadCryptoCreate.ChaCha20Poly1305(key), nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
	}

	[Test]
	public async Task ManagedSupportsUnalignedSegments()
	{
		byte[] key = RandomNumberGenerator.GetBytes(ChaCha20Poly1305Crypto.KeySize);
		byte[] nonce = RandomNumberGenerator.GetBytes(ChaCha20Poly1305Crypto.NonceSize);
		byte[] associatedData = RandomNumberGenerator.GetBytes(29);
		byte[] plaintext = RandomNumberGenerator.GetBytes(1031);
		byte[] expectedCiphertext = new byte[plaintext.Length];
		byte[] expectedTag = new byte[ChaCha20Poly1305Crypto.TagSize];
		byte[] ciphertext = new byte[plaintext.Length];
		byte[] tag = new byte[ChaCha20Poly1305Crypto.TagSize];
		byte[] decrypted = new byte[plaintext.Length];

		using ChaCha20Poly1305 expected = new(key);
		expected.Encrypt(nonce, plaintext, expectedCiphertext, expectedTag, associatedData);

		using ChaCha20Poly1305Crypto crypto = new(key);
		crypto.Encrypt(nonce, plaintext, ciphertext, tag, associatedData);
		crypto.Decrypt(nonce, ciphertext, tag, decrypted, associatedData);

		await Assert.That(ciphertext).IsEquivalentTo(expectedCiphertext, CollectionOrdering.Matching);
		await Assert.That(tag).IsEquivalentTo(expectedTag, CollectionOrdering.Matching);
		await Assert.That(decrypted).IsEquivalentTo(plaintext, CollectionOrdering.Matching);
	}

	[Test]
	[Arguments(ChaCha20Poly1305Crypto.TagSize - 1)]
	[Arguments(ChaCha20Poly1305Crypto.TagSize + 1)]
	[SuppressMessage("ReSharper", "AccessToDisposedClosure")]
	public async Task InvalidTagLengthDoesNotWriteAndPreservesInPlace(int tagLength)
	{
		byte[] key = TestUtils.CreateDeterministicSource(ChaCha20Poly1305Crypto.KeySize);
		byte[] nonce = TestUtils.CreateDeterministicSource(ChaCha20Poly1305Crypto.NonceSize);
		byte[] associatedData = TestUtils.CreateDeterministicSource(29);
		byte[] plaintext = TestUtils.CreateDeterministicSource(73);
		byte[] invalidTag = new byte[tagLength];
		TestUtils.PrepareDestination(invalidTag);

		using ChaCha20Poly1305Crypto crypto = new(key);

		byte[] encryptBuffer = plaintext.ToArray();
		await Assert.That(() => crypto.Encrypt(nonce, encryptBuffer, encryptBuffer, invalidTag, associatedData)).ThrowsExactly<ArgumentOutOfRangeException>();
		await Assert.That(encryptBuffer).IsEquivalentTo(plaintext, CollectionOrdering.Matching);
		await Assert.That(invalidTag).All(static value => value is TestUtils.DestinationSentinel);

		byte[] decryptBuffer = TestUtils.CreateDeterministicSource(plaintext.Length);
		byte[] ciphertext = decryptBuffer.ToArray();
		await Assert.That(() => crypto.Decrypt(nonce, decryptBuffer, invalidTag, decryptBuffer, associatedData)).ThrowsExactly<ArgumentOutOfRangeException>();
		await Assert.That(decryptBuffer).IsEquivalentTo(ciphertext, CollectionOrdering.Matching);
		await Assert.That(invalidTag).All(static value => value is TestUtils.DestinationSentinel);

		byte[] roundTripBuffer = plaintext.ToArray();
		byte[] tag = new byte[ChaCha20Poly1305Crypto.TagSize];
		crypto.Encrypt(nonce, roundTripBuffer, roundTripBuffer, tag, associatedData);
		crypto.Decrypt(nonce, roundTripBuffer, tag, roundTripBuffer, associatedData);

		await Assert.That(roundTripBuffer).IsEquivalentTo(plaintext, CollectionOrdering.Matching);
	}
}
