...
public class AesGcmTest
{
	...
	{
		await crypto.AeadTest(@"AES-GCM", nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
	}
	...
	public async Task Test(string keyHex, string nonceHex, string associatedDataHex, string tagHex, string plainHex, string cipherHex)
	{
		...
		await Test_Internal(new DefaultAesGcmCrypto(key), nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
		await Test_Internal(AeadCryptoCreate.AesGcm(key), nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
		await Test_Internal(new GcmMode128<AesCipher>(AesCipher.Create(key)), nonceHex, associatedDataHex, tagHex, plainHex, cipherHex);
	}

	[Test]
	public async Task InPlaceDecryptionAuthenticatesBeforeWritingPlaintext()
	{
		byte[] key = CreateDeterministicSource(16);
		byte[] nonce = CreateDeterministicSource(GcmMode128<AesCipher>.NonceSize);
		byte[] associatedData = CreateDeterministicSource(37);
		byte[] plaintext = CreateDeterministicSource(1031);
		byte[] buffer = plaintext.ToArray();
		byte[] tag = new byte[GcmMode128<AesCipher>.TagSize];
		using GcmMode128<AesCipher> gcm = new(AesCipher.Create(key));
		...
		await Assert.That(() => gcm.Decrypt(nonce, buffer, invalidTag, buffer, associatedData)).Throws<AuthenticationTagMismatchException>();
		await Assert.That(buffer).IsEquivalentTo(ciphertext, CollectionOrdering.Matching);

		gcm.Decrypt(nonce, buffer, tag, buffer, associatedData);
	...
	[Arguments(15)]
	[Arguments(17)]
	public async Task TagLengthMustBeExactAndRejectedCallsDoNotChangeStateOrOutput(int tagLength)
	{
		byte[] key = CreateDeterministicSource(16);
		byte[] nonce = CreateDeterministicSource(GcmMode128<AesCipher>.NonceSize);
		byte[] associatedData = CreateDeterministicSource(37);
		byte[] plaintext = CreateDeterministicSource(65);
		...
		{
			reference.Encrypt(nonce, plaintext, expectedCiphertext, expectedTag, associatedData);
		}

		using GcmMode128<AesCipher> gcm = new(AesCipher.Create(key));
		byte[] invalidEncryptionDestination = new byte[plaintext.Length];
		byte[] invalidEncryptionTag = new byte[tagLength];
		PrepareDestination(invalidEncryptionDestination);
		PrepareDestination(invalidEncryptionTag);

		await Assert.That(() => gcm.Encrypt(nonce, plaintext, invalidEncryptionDestination, invalidEncryptionTag, associatedData)).ThrowsExactly<ArgumentOutOfRangeException>();
		await Assert.That(invalidEncryptionDestination).All(static value => value is DestinationSentinel);
		await Assert.That(invalidEncryptionTag).All(static value => value is DestinationSentinel);

		byte[] actualCiphertext = new byte[plaintext.Length];
		byte[] actualTag = new byte[GcmMode128<AesCipher>.TagSize];
		gcm.Encrypt(nonce, plaintext, actualCiphertext, actualTag, associatedData);
		await Assert.That(actualCiphertext).IsEquivalentTo(expectedCiphertext, CollectionOrdering.Matching);
		await Assert.That(actualTag).IsEquivalentTo(expectedTag, CollectionOrdering.Matching);

		byte[] invalidDecryptionDestination = new byte[plaintext.Length];
		byte[] invalidDecryptionTag = new byte[tagLength];
		expectedTag.AsSpan().Slice(0, Math.Min(expectedTag.Length, invalidDecryptionTag.Length))
			.CopyTo(invalidDecryptionTag);
		byte[] invalidDecryptionTagCopy = invalidDecryptionTag.ToArray();
		PrepareDestination(invalidDecryptionDestination);

		await Assert.That(() => gcm.Decrypt(nonce, expectedCiphertext, invalidDecryptionTag, invalidDecryptionDestination, associatedData)).ThrowsExactly<ArgumentOutOfRangeException>();
		await Assert.That(invalidDecryptionDestination).All(static value => value is DestinationSentinel);
		await Assert.That(invalidDecryptionTag).IsEquivalentTo(invalidDecryptionTagCopy, CollectionOrdering.Matching);

		byte[] actualPlaintext = new byte[plaintext.Length];
		gcm.Decrypt(nonce, expectedCiphertext, expectedTag, actualPlaintext, associatedData);
		await Assert.That(actualPlaintext).IsEquivalentTo(plaintext, CollectionOrdering.Matching);
	}
}
