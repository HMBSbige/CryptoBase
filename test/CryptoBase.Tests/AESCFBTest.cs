using CryptoBase.Abstractions.SymmetricCryptos;
using CryptoBase.DataFormatExtensions;
using CryptoBase.SymmetricCryptos.StreamCryptos;

namespace CryptoBase.Tests;

public class AESCFBTest
{
	private static async Task Test_Internal(IStreamCrypto crypto, string hex, string hex2)
	{
		await Assert.That(crypto.Name).IsEqualTo(@"AES-CFB");

		byte[] h1 = hex.FromHex();

		byte[] h2 = hex2.FromHex();

		byte[] o = new byte[h1.Length];

		crypto.Update(h1, o);
		await Assert.That(h2.SequenceEqual(o)).IsTrue();

		crypto.Reset();

		crypto.Update(h1.AsSpan().Slice(0, 73), o);
		crypto.Update(h1.AsSpan().Slice(73), o.AsSpan().Slice(73));
		await Assert.That(h2.SequenceEqual(o)).IsTrue();

		crypto.Reset();

		for (int i = 0; i < h1.Length; ++i)
		{
			crypto.Update(h1.AsSpan().Slice(i, 1), o.AsSpan().Slice(i, 1));
		}

		await Assert.That(h2.SequenceEqual(o)).IsTrue();

		crypto.Dispose();
	}

	/// <summary>
	/// https://gchq.github.io/CyberChef/#recipe=AES_Encrypt(%7B'option':'Hex','string':''%7D,%7B'option':'Hex','string':''%7D,'CFB','Hex','Hex','')AES_Decrypt(%7B'option':'Hex','string':''%7D,%7B'option':'Hex','string':''%7D,'CFB','Hex','Hex',%7B'option':'Hex','string':''%7D,'')
	/// </summary>
	[Test]
	[Arguments(@"A0B0C0D0E0F1011121314151617181AA",
		@"fe3577795961e1fbbbb82528c74d2e99",
		@"",
		@"")]
	[Arguments(@"000102030405060708090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F",
		@"00112233445566778899aabbccddeeff",
		@"",
		@"")]
	[Arguments(@"08090A0B0C0D0E0F101112131415161718191A1B1C1D1E1F",
		@"00112233445566778899aabbccddeeff",
		@"",
		@"")]
	public async Task Test(string keyHex, string ivHex, string hex, string hex2)
	{
		byte[] key = keyHex.FromHex();
		byte[] iv = ivHex.FromHex();
		await Test_Internal(StreamCryptoCreate.AesCfb(true, key, iv), hex, hex2);
		await Test_Internal(StreamCryptoCreate.AesCfb(false, key, iv), hex2, hex);
	}
}
